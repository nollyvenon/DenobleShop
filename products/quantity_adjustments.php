...
$sess_warehouse_id = $_SESSION['warehouse_id'];//get the warehouse_keeper warehouse id
$adjust_list = $zentabooks_operation->get_warehouse_adjustments($sess_warehouse_id);
//print_r($adjust_list);
?><!DOCTYPE html>
<html>
<head>
    ...
    <title>Quantity Adjustment- ZentaBooks</title>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-filter"></i>Quantity Adjustments <?php if($hiss == ""){?> (All Warehouses) <?php }else{ echo '('.$mywarehouse.')';}?> </h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?php echo SITE_URL;?>/products/add_adjustment">
                                <i class="fa fa-plus-circle"></i> Add Adjustment                            </a>
                        </li>
                       <!-- <li>
                            <a href="<?php echo SITE_URL;?>/products/add_adjustment_by_csv">
                                <i class="fa fa-plus-circle"></i> Add Adjustment by CSV                            </a>
                        </li>
                        <li>
                ...
                <?php if($admin_type=='super-admin'){ ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?php echo SITE_URL;?>/products/quantity_adjustments"><i class="fa fa-building-o"></i> All Warehouses</a></li>
                            <li class="divider"></li>
                          <?php  foreach ($allwarehouses as $row) {   ?>
                            <li><a href="<?php echo SITE_URL;?>/products/quantity_adjustments?hiss=<?=$row['id'];?>"><i class="fa fa-building"></i><?=$row['name'];?></a></li>    
                          <?php } ?>                  
                        </ul>
                    </li>
                <?php } ?>
            ...
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="hidden-table-info" class="table table-bordered table-condensed table-hover table-striped">
                        ...
                        <tbody>
                        <?php if(isset($adjust_list) && !empty($adjust_list)) { foreach ($adjust_list as $row) {
							$user_det = $zentabooks_operation->get_user_details($row['created_by']); ?>
                            <tr>
                                <td><input class="checkbox multi-select" type="checkbox" name="check"/></td>
                                <td><?php echo $row['date']; ?></td>
                                <td align="center"><?php echo $row['reference_no']; ?></td>
                                <td><?php echo $zentabooks_operation->get_warehouse_name_by_id($row['warehouse_id']); ?></td>
                                <td><?php echo $user_det[0]['first_name'].' '.$user_det[0]['last_name']; ?></td>
                                <td><?php echo htmlspecialchars_decode($row['note']); ?></td>
                              	<td></td>
                              <td><div class="dropdown">
  <button class="btn btn-primary btn-xs dropdown-toggle" type="button" data-toggle="dropdown">Actions  
  <span class="caret"></span></button>
  <ul class="dropdown-menu">
    <li><a href="<?php echo SITE_URL;?>/products/adjustment_view?hiss=<?php echo encrypt($row['id']); ?>"><i class="fa fa-plus-square"></i> View</a></li> 
    <li><a href="<?php echo SITE_URL;?>/products/adjustment_edit?hiss=<?php echo encrypt($row['id']); ?>"><i class="fa fa-plus-square"></i> Edit</a></li> 
