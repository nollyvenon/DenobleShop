...
if (!contains( '11',$allowed_pages)) {
	 $message_error .= "You do not have right to that page or feature. Regards.";
	redirect_to("../dashboard?msg=10");
 }
if (!empty($_SESSION['salespoint_id'])){
	$salespoint_id = $_SESSION['salespoint_id'];
}
$mysalespoint = $zentabooks_operation->get_salespoint_name_by_id($salespoint_id);
...
$refno = "SALE/".date('Y').'/'.date('m').'/'.sprintf("%04s", $trans_count);
	 ...
	 if ($_POST['add_sale']){
		...
		if ($_POST['date'] != ""){
		$myDateTime1 = DateTime::createFromFormat('d/m/Y H:i:s', $_POST['date']);
		...
		$amountpaid=$_POST['amountpaid'] + $pos_amount + $cash_amount;
		...
		foreach($product as $a => $b){//first runa check to be sure that the stock is available in the warehouse and system
		$product_name = $zentabooks_operation->view_product_detail($product[$a])['name'];
			$stock_qty = $zentabooks_operation->get_salespoint_qty_by_productid($product[$a],$salespoint);//get current stock amount
			if ($stock_qty - $quantity[$a] < 0){
				$message_error .= "$product_name's stock isn't enough for the transaction, kindly contact the warehouse and store.<br>";
			}
		}
		if ($message_error ==""){//stock is available for the entire sale transaction
			 $document  = time().$_FILES['document']['name'];
			  ...
			  foreach($product as $a => $b){
				 // $totamount += $subtotal[$a];
				  ...
				  $profit = ($prod_det['price'] -$prod_det['cost'])*$quantity[$a];
				  $totamount += $prod_det['price']*$quantity[$a];
				  $totprofit += $profit;
				  $tot_order_discount += $discount[$a];
			  	  $product_incentive_ratio = $zentabooks_operation->view_product_detail($product[$a])['incentive_ratio'];
				  $prod_comm = $product_incentive_ratio*$profit;
				  $zentabooks_operation->add_sale_items($sale_id, $salespoint, $product[$a],  $serialnumber[$a], intval($quantity[$a]), $netunitprice[$a], $discount[$a], $producttax[$a], $subtotal[$a], $prod_comm);
				  $salescomm +=$prod_comm;
			  }
			  $settings  = $zentabooks_operation->get_settings();
			  
			  $tot_discount = $order_discount + $tot_order_discount;
			  
			  $query = "UPDATE sma_sales SET total_items='$noofitems', total_discount='$tot_discount', grand_total='$totamount', salescomm='$salescomm' WHERE id='$sale_id'";
			  $db_handle->runQuery($query);
			  if ($payment_status=='partial' ||$payment_status=='paid'){
				  if ($paid_by == 'cash_pos'){
					  $zentabooks_operation->add_sale_payments($date, $reference_no, $sale_id, $salespoint, 'cash', $cheque_no, $pcc_no, $pcc_holder, $pcc_type, $pcc_year, $pcc_ccv, $cash_amount, $currency, $created_by, $attachment, $sale_status, $note);
					  $zentabooks_operation->add_sale_payments($date, $reference_no, $sale_id, $salespoint, 'pos', $cheque_no, $pcc_no, $pcc_holder, $pcc_type, $pcc_year, $pcc_ccv, $pos_amount, $currency, $created_by, $attachment, $sale_status, $note);
				  }else{
					  $zentabooks_operation->add_sale_payments($date, $reference_no, $sale_id, $salespoint, $paid_by, $cheque_no, $pcc_no, $pcc_holder, $pcc_type, $pcc_year, $pcc_ccv, $amountpaid, $currency, $created_by, $attachment, $sale_status, $note);
				  }
			  }
...
<head>
    ...
    <![endif]-->
    <!--<link href="<?php echo SITE_URL;?>/themes/default/assets/styles/jquery.flexdatalist.min.css" rel="stylesheet" type="text/css">
    <script src="<?php echo SITE_URL;?>/themes/default/assets/js/jquery.flexdatalist.min.js"></script>-->
    <noscript><style type="text/css">#loading { display: none; }
    .row.no-gutter {
  margin-left: 0;
  margin-right: 0;
}
...
function check_dd1() {
	...
	if(document.getElementById('paid_by_1').value == "cash_pos") {
        document.getElementById('ygc').style.display = 'block';
        document.getElementById('gc').style.display = 'none';
        document.getElementById('ngc').style.display = 'none';
    } else {
         document.getElementById('ygc').style.display = 'none';
       document.getElementById('gc').style.display = 'none';
        document.getElementById('ngc').style.display = 'block';
    }
			...
			var quantity1 = name1+1;
			 ...
			 else
			 {
			  ...
			  tax1="0000000";
				 ...
				 $.post("<?php echo SITE_URL;?>/sales/check_sales_details.php", {stock_name: $("#"+idname).val(), custtype:$("#custtype").val(),depot:$("#location").val(), territ:$("#territ").val() },			
				 	function(data){
								
								$("#"+name1).val($("#"+idname).val());
								$("#"+rate1).val(data.rate);
								$("#"+avail1).val(data.availstock);
								$("#"+tax1).val(data.tax);
								$("#"+quantity1).val(data.serial_no);
								//$("#"+quantity1).focus();
							}, 'json');
    ...
    var total = 0;
    $('tr').each(function() {
       // $(this).find('span.amount').html($('input:eq(1)', this).val() * $('input:eq(2)', this).val() - $('input:eq(3)', this).val());
        var subtotal = $('input:eq(2)', this).val() * $('input:eq(3)', this).val() - $('input:eq(4)', this).val();
		$(this).find('#amount1').val(subtotal);
		$(this).find('div.amount2').html(subtotal);
		//$(this).find('span.amount').html(Math.round(subtotal).toLocaleString('en-US'));
		$(this).find('span.amount').html(accounting.formatMoney(subtotal, "₦", 2, ",", "."));
    });
    $('.amount2').each(function() {
        total += parseInt($(this).text(),10);
    });
    $('div.total_amount').html(accounting.formatMoney(total, "₦", 2, ",", "."));
}
//$('button').click(doCalc);
                ...
                <div class="row">
                    <div class="col-lg-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="date" value="<?= date('d/m/Y H:i:s', time());?>"  class="form-control input-tip datetime" id="date" required="required" />
                                </div>
                            </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <?php }else{ ?>
                            <input type="hidden" name="biller" value="<?php echo $_SESSION['biller_id'];?>">
                            <?php } ?>
                            ...
                            <div class="panel panel-warning">
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="salespoint" id="slsalespoint" class="form-control col-sm-5" >
                                                    <?php if($_SESSION['salespoint_id'] != ""){ ?>
                                                     <option selected value="<?php echo $_SESSION['salespoint_id'];?>"><?php echo $mysalespoint;?></option>
                                            	<?php  }else{ ?>
                                                 <option value="" >Select Salespoint...</option>
      											 <?php foreach($allsalespoints as $row2): ?>
                                                      <option <?php if($_SESSION['salespoint_id'] == $row2['id']){ echo 'selected';  } ?> value="<?php echo $row2['id'];?>">
                                                                <?php echo $row2['name'];?>
                                                                    </option>
                                                    <?php endforeach; ?>
                                                <?php  } ?>
                                        ...
                                        <div class="form-group">
                                            <label for="slcustomer">Customer</label>                                            <div class="input-group">
                                                ...
                                                <!--<input type="text" name="customer" id="0" list="customername" class=" col-md-12 form-control">
                                                <datalist id="customername"><option>Select A Customer</option>
                 ...
                 <div class="table-responsive">  
                   ...
                   <thead> <tr class="title">
                      <th width="30%">Product (Code - Name)<span class="required">*</span></th>
                      <th width="15%">Serial No</th>
                      <th width="9%"> Price</th>
                      <th width="7%">Quantity</th>
                      <th width="9%">Discount</th>
                      <th width="9%">Product Tax</th>
                      <th width="12%">Subtotal (NGN)</th>
                      <th width="7%">Action</th>
                    </tr>
                    </thead>
                  </table>
                  <table align="left" border="0" cellspacing="2" cellpadding="2"  id="duplicate">
  						<tr>
  						  <td width="30%"><input type="text" name="product[]" id="0" list="pname" onBlur="callAutoAsignValue(this.id)" onChange="callAutoAsignValue(this.id)"  class=" col-md-12 form-control flexdatalist">
                  			<datalist id="pname"><option>Select A Product</option>
                           <?php  foreach ($allproducts as $row) {   ?>
                                    <option  value="<?=$row['name'];?>"><?=$row['name'];?></option>
                                    <?php } ?>
            </datalist><!--<select name="product[]" id="0"  onBlur="callAutoAsignValue(this.id)" onChange="callAutoAsignValue(this.id)"  class=" col-md-12 form-control">
                      ...
                      <td width="15%"><input name="serialnumber[]" id="00"   class=" col-md-12 form-control" type="text" size="35" /></td>
                      <td width="9%"><input name="netunitprice[]" type="text"  required class=" col-md-12 form-control" id="000" size="35" readonly /></td>
					  <td width="7%"><input name="quantity[]" id="0000" required class=" col-md-12 form-control" type="text" size="35"  onfocus="doCalc()" onchange="doCalc()"/> </td>
                      <td width="9%"><input name="discount[]" id="00000"   class=" col-md-12 form-control" type="text" size="35" onfocus="doCalc()" onchange="doCalc()"/></td>
                      <td width="9%"><input name="producttax[]" id="000000"   class=" col-md-12 form-control" type="text" size="35" />  </td>
                      <td width="12%" style="text-align:center"><span class="amount">₦0.00</span> <div class="amount2" style="display:none"></div> <!--<input name="amount1[]" id="amount1" value="0" type="textfield">--> </td>
                      <td width="7%">&nbsp;<a id="minus" href=""  ><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></a> <a id="plus" href="">[+]</a></td>
                    </tr>
					  <tr>
					    <td>&nbsp;</td>
						<td>&nbsp;</td>
						...
						<td>&nbsp;</td>
                                ...
                                <div class="form-group">
                                    <label for="sltax2">Order Tax</label>                                    <select name="order_tax_id" id="sltax2" data-placeholder="Select Order Tax" class="form-control input-tip select" style="width:100%;">
<option value=""></option>
...
<option value="paid">Paid</option>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="payment">
                                                    ...
                                                    <div id="ygc" style="display: none;">
                                                        <div class="form-group ygc">
                                                            <label for="amount_1">Cash Amount</label>                                                        
                                                            <input name="cash_amount" type="text" id="amount_1"
                                                                   class="pa form-control kb-pad amount"/>
                                                        </div>
                                                        <div class="form-group ygc">
                                                            <label for="amount_1">POS Amount</label>                                                        
                                                            <input name="pos_amount" type="text" id="amount_1"
                                                                   class="pa form-control kb-pad amount"/>
                                                        </div>
                                                    </div>
        ...
        <option value="pos">Direct Transfer</option>
                ...
                <div id="bottom-total" class="well well-sm" style="margin-bottom: 0;">
                    <table class="table table-bordered table-condensed totals" style="margin-bottom:0;">
                        <tr class="warning">
                            ...
                            <td>Grand Total &nbsp;&nbsp;<div class="total_amount pull-right">total</div></td>
                        </tr>
                    </table>
                </div>
