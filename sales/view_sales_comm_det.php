...
$id_encrypted = decrypt(str_replace(" ", "+", $db_handle->sanitizePost($_GET['hiss'])));
$hiss = preg_replace("/[^A-Za-z0-9 ]/", '', $id_encrypted);
$id_encrypted = decrypt(str_replace(" ", "+", $db_handle->sanitizePost($_GET['st'])));
$month = preg_replace("/[^A-Za-z0-9 ]/", '', $id_encrypted);
$id_encrypted = decrypt(str_replace(" ", "+", $db_handle->sanitizePost($_GET['sf'])));
$year = preg_replace("/[^A-Za-z0-9 ]/", '', $id_encrypted);
$mysalespoint = $zentabooks_operation->get_salespoint_by_id($salespoint_id)['name'];
...
<head>
    ...
    <title>Sales Commission Log - ZentaBooks</title>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-heart"></i>Sales Commission Log<?php if($salespoint_id == ""){?> (All Salespoints) <?php }else{ echo '('.$mysalespoint.')';}?>       </h2>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table  id="hidden-table-info1" class="table table-bordered table-hover table-striped">
                        <thead>
                        <tr>
                            <th>Date</th>
                            <th>Salespoint</th>
                        ...
                        <tbody>
							<?php 
							$payment_list = $zentabooks_operation->sales_commission_det1("", $month, $year);
							if(isset($payment_list) && !empty($payment_list)) { foreach ($payment_list as $row) { 
							 $creator = $zentabooks_operation->get_user_details($row['user_id'])[0];
							 $myDateTime1 = DateTime::createFromFormat('m', $row['month']);
							 $month = $myDateTime1->format('M');
							?>
                            <tr>
                                <td><?php echo $month.', '.$row['year']; ?></td>
                                <td><?php echo $zentabooks_operation->get_salespoint_by_id($row['salespoint'])['name']; ?></td>
                                <td><?php echo $creator['first_name'].' '.$creator['last_name']; ?></td>
                                <td><?php echo number_format($row['amount'],2); ?></td>
                              <td><a class="btn btn-warning btn-xs" href="<?php echo SITE_URL;?>/sales/view_sales_comm_det?hiss=<?php echo encrypt($row['id']); ?>"><i class="fa fa-plus-square"></i> Details</a></td>
                            </tr>
