...
$hiss = $_GET['id'];
$mystore = $zentabooks_operation->get_store_name_by_id($hiss);
$view_loyalties = $zentabooks_operation->view_loyalties_list();
$customers = $zentabooks_operation->customers_list();
?><!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <title>View Loyalty Card - ZentaBooks</title>
            ...
            <div id="content">
                <div class="row">
                    <div class="col-sm-12 col-md-12">
                        <ul class="breadcrumb">
                            <li><a href="<?php echo SITE_URL;?>/">Home</a></li><li><a href="<?php echo SITE_URL;?>/hrmanager">HR Manager</a></li><li class="active">View Loyalty Card</li>                            <li class="right_log hidden-xs">
...
xmlhttp.open("GET","<?php echo SITE_URL;?>/sales/loyaltycarddet.php?q="+str,true);
xmlhttp.send();
}

function showTerri2(str,str1,str2)
{
	if (str=="") {
	  document.getElementById("txtHint1").innerHTML="";
	  return;
	...
	//xmlhttp.open("GET","http://localhost/optisys/getitemprice.php?q="+str,true);
	xmlhttp.open("GET","<?php echo SITE_URL;?>/sales/loyaltycarddet.php?q="+str+"&r="+str1+"&s="+str2,true);
	xmlhttp.send();
}
...
<form action="<?php echo SITE_URL;?>/reports/markdailysalescomplete" data-toggle="validator" role="form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                                                                           <input type="hidden" name="token" value="125b3ac26a368a11016f2c4c904aba72" />
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label for="Customer">Customer</label>
                                <select name="customer_id"   class="form-control input-tip select" onfocus="showTerri(this.value,'','');" onchange="showTerri(this.value,'','');">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input name="start_date" id="start_date" value="" class="form-control datetime" type="text"  onfocus="showTerri2(customer_id.value,this.value,'');" onchange="showTerri2(customer_id.value,this.value,'');">
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label for="end_date">End Date</label>                                
                                <input name="end_date" id="end_date" value="" class="form-control datetime" type="text" onfocus="showTerri2(customer_id.value,start_date.value,this.value);" onchange="showTerri2(customer_id.value,start_date.value,this.value);">
                            </div>
                        </div>
<div id="txtHint1" class="table-responsive"><table id="" class="table table-bordered table-hover table-striped">
                            <thead>
                            <tr>
                                <th> ID </th>
                                <th>Trans Date </th>
                            ...
                            <tbody>
                            <?php 
							if(isset($view_loyalties) && !empty($view_loyalties)) { foreach ($view_loyalties as $row) { ?>
                                <tr align="center">
                                <td> <?php echo $row['cum_loyalty_id']; ?></td>
                                <td>  <?php echo $row['timestamp']; ?> </td>
                                <td>  <?php echo $row['customer_name']; ?> </td>
                                <!--<td> <a target='_blank' href=sales/view/122>122</a> </td>-->
                                <td>  <?php echo formatMoney($row['totamount'],2); ?></td>
                                <td>  <?php echo formatMoney($row['points'],2); ?></td>
                                <td> 1 </td>
                                <td> <a target='_blank' href="sales/viewloyaltycarddet?hiss=<?php echo $row['customer_id'];?>">Details</a> </td>
                              </tr>
                        	<?php $totamt += $row['amount'];
...
<a href="#" id="toTop" class="blue" style="position: fixed; bottom: 30px; right: 30px; font-size: 30px; display: none;">
    ...
    <p style="text-align:center;">&copy; 2017 DeNobelAS 
      - Page rendered in <strong>0.6700</strong> seconds</p>
</footer>
...
<script type="text/javascript" charset="UTF-8">var oTable = '', r_u_sure = "Are you Sure";
    ...
    $(window).load(function () {
        $('.mm_hrmanager').addClass('active');
        $('.mm_hrmanager').find("ul").first().slideToggle();
        $('#hrmanager_viewloyaltycard').addClass('active');
        $('.mm_hrmanager a .chevron').removeClass("closed").addClass("opened");
    });
</script>
</body>
</html>
